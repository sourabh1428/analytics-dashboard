  ...
  return (
    <div className="bg-white">
      ...
      <section className="bg-gradient-to-b from-violet-50 to-white py-24 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              <span className="inline-flex items-center gap-2 rounded-full bg-violet-100 px-3 py-1 text-xs font-semibold text-violet-700 mb-6">
                <Bell className="h-3 w-3" /> Refill Reminders
              </span>
              <h1 className="text-4xl md:text-5xl font-bold text-slate-900 leading-tight mb-6">
                Patients who forget to refill are just{" "}
                <span className="text-violet-600">one message away</span>
              </h1>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Link
                  ...
                  className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-violet-600 text-white font-semibold hover:bg-violet-700 transition-colors"
                >
              ...
              <div className="bg-gray-900 rounded-3xl p-6 shadow-2xl max-w-sm mx-auto">
                ...
                {[
                  ...
                  <div key={i} className="flex items-center gap-3 py-3 border-b border-white/5 last:border-0">
                    <div className="h-8 w-8 rounded-full bg-violet-500/20 flex items-center justify-center text-xs font-bold text-violet-300 shrink-0">
                      {p.name[0]}
                    </div>
                ...
                <div className="mt-4 rounded-xl bg-violet-600/20 border border-violet-500/30 p-3">
                  <p className="text-violet-300 text-xs font-semibold">Today's reminder queue</p>
        ...
        <div className="max-w-5xl mx-auto grid sm:grid-cols-3 gap-8 text-center">
          {[
            ...
            <div key={s.label}>
              ...
              <p className="text-violet-200 text-sm mt-2">{s.label}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          <div className="space-y-8">
            {steps.map((s) => (
              <div key={s.step} className="flex gap-6 items-start">
                <div className="h-10 w-10 rounded-full bg-violet-100 text-violet-700 font-bold flex items-center justify-center shrink-0">
                  {s.step}
                </div>
                <div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <div key={title} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                <Icon className="h-6 w-6 text-violet-600 mb-4" />
      ...
      <section className="py-20 px-6 bg-slate-900 text-center">
        ...
        <Link href="https://dashboard.easibill.com/" className="inline-flex items-center gap-2 px-8 py-4 rounded-xl bg-violet-600 text-white font-semibold hover:bg-violet-500 transition-colors">
